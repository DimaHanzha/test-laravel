                    ...
                    <div class="card-body">
                        <form action="{{route('author.update', $author->id)}}" method="post">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input type="text" class="form-control" id="first-name" placeholder="" name="first_name" value="{{$author->first_name}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="last_name" class="col-md-3 col-form-label text-md-right">Last name: </label>
                                <div class="col-md-6">
                                    <input type="text" class="form-control" id="last-name" placeholder="" name="last_name" value="{{$author->last_name}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <select name="books[]" multiple="multiple" class="form-control select2">
                                        @foreach($books as $book)
                                            <option
                                                {{in_array($book->id, $author->books->pluck('id')->toArray())? 'selected' : ''}} value="{{$book->id}}">{{$book->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
